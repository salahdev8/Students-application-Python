import tkinter as tk
from tkinter import ttk

import StudentApp
from utils import database

class StudentGrades(tk.Frame):

    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)

        self.studentName = tk.StringVar()
        self.password = tk.StringVar()
        self.CourseName = tk.StringVar()



        self.operationsFrame = tk.Frame(self)

        self.initComponents(controller)

    def initComponents(self, controller):
        buttonFrame = tk.Frame(self)


        tk.Label(self.operationsFrame, text="Enter Your Last name :").grid(column=0, row=0, pady=4)
        tk.Entry(self.operationsFrame, width=25, textvariable=self.studentName).grid(column=1, row=0, pady=4)
        tk.Label(self.operationsFrame, text="Enter your password :").grid(column=0, row=1, pady=4)
        tk.Entry(self.operationsFrame, width=25, textvariable=self.password).grid(column=1, row=1, pady=4)

        tk.Button(self.operationsFrame, text="submit", command=self.show_courses,
                  borderwidth=3, width=7,
                  height=1).grid(column=2, row=2, pady=10)




        b1 = tk.Button(buttonFrame, text="Lecture Files ", command=lambda: controller.show_frame(StudentApp.CheckFiles),borderwidth=4, width=24, height=3)
        b2 = tk.Button(buttonFrame, text="Assignments",command=lambda: controller.show_frame(StudentApp.StudentAssignment), borderwidth=4, width=24,height=3)
        b3 = tk.Button(buttonFrame, text="Grades", command=lambda: controller.show_frame(StudentApp.StudentGrades),borderwidth=4, width=24, height=3)
        b4 = tk.Button(buttonFrame, text="Announcements",
                       command=lambda: controller.show_frame(StudentApp.StudentAnnouncements), borderwidth=4, width=24,
                       height=3)
        b5 = tk.Button(buttonFrame, text="Log Out", command=lambda: controller.show_frame(StudentApp.LoginWindow),
                       borderwidth=4, width=24, height=3)
        b1.grid(column=0, row=0)
        b2.grid(column=0, row=1)
        b3.grid(column=0, row=2)
        b4.grid(column=0, row=3)
        b5.grid(column=0, row=5)

        buttonFrame.grid(column=0, row=0)
        self.operationsFrame.grid(column=1, row=0, padx=5)

    def show_courses(self):
        courseslist = database.select_student_course(self.studentName.get(),self.password.get())

        tk.Label(self.operationsFrame, text="Choose a course  :").grid(column=0, row=3)
        tk.Spinbox(self.operationsFrame, fg="blue", font=12, textvariable=self.CourseName, values=courseslist).grid(
            column=1, row=3)
        tk.Button(self.operationsFrame, text="check grades",command=self.show_grades,
                  borderwidth=3, width=9,
                  height=1).grid(column=3, row=3, pady=10)

    def show_grades(self):

        cid=database.course_id(self.CourseName.get())
        assid=database.select_assignment_ids(cid)

        tk.Label(self.operationsFrame, text="assignments").grid(column=0, row=4)
        tk.Label(self.operationsFrame, text="grade").grid(column=1, row=4)
        tk.Label(self.operationsFrame, text="description").grid(column=2, row=4)
        gradesandremark=database.select_course_grades(assid)
        homeworks=database.select_assignment_names(assid)

        i=5
        for ass in homeworks:
            tk.Label(self.operationsFrame, text=ass).grid(column=0, row=i)
            i+=1
        j=5
        e=0
        while(e < len(gradesandremark)):
            tk.Label(self.operationsFrame,text=gradesandremark[e]).grid(column=1,row=j)
            tk.Label(self.operationsFrame, text=gradesandremark[e+1]).grid(column=2, row=j)
            e+=2
            j+=1







# if __name__ == "__main__":
#     root = tk.Tk()
#     root.title("Admin")
#     CourseOperations(root)
#     root.mainloop()
