...
class CourseAssignments(tk.Frame):
    ...
    def teacherOperations(self, controller):
        ...
        tk.Spinbox(self.operationsFrame, fg="blue", font=12, textvariable=self.CourseName, values=courseNames).grid(
            column=1, row=0)
        tk.Label(self.operationsFrame, text="Assignment Name :").grid(column=0, row=1, pady=4)
        tk.Entry(self.operationsFrame, width=25, textvariable=self.Assignment).grid(column=1, row=1, pady=4)

        tk.Label(self.operationsFrame, text="Assignment File :").grid(column=0, row=2, pady=4)
        ...
        tk.Button(self.operationsFrame, text="Submit",
                  command=lambda: controller.add_assignemnt_file(database.course_id(self.CourseName.get()),
    ...
    #     self.CourseName=self.CourseName.get()
    #     self.cid = database.course_id(self.CourseName)
    #     self.label3 = tk.Label(self.operationsFrame, text="")
    #     self.label3.grid(column=0, row=7, pady=4)
    #     self.label3.configure(text=database.display_assignments(self.cid))
    #
    #
