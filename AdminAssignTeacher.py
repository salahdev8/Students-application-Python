...
class AdminAssignTeacher(ttk.Frame):
    def __init__(self, parent, controller):
        ttk.Frame.__init__(self, parent)

        self.CourseName = tk.StringVar()
        self.TeacherName = tk.StringVar()
    ...
    def adminOperations(self, controller):
        ...
        teacherNames=database.select_teachers()


        tk.Label(operationsFrame, text="Course Name :").grid(column=0, row=0)
        tk.Spinbox(operationsFrame, fg="blue", font=12,textvariable=self.CourseName, values=courseNames).grid(column=1, row=0)

        tk.Label(operationsFrame, text="Teacher Name :").grid(column=0, row=1)
        tk.Spinbox(operationsFrame, fg="blue", font=12, textvariable=self.TeacherName, values=teacherNames).grid(column=1,row=1)

        tk.Button(operationsFrame, text="submit",
                  command=lambda: controller.assign_teacher(self.CourseName.get(), self.TeacherName.get()), borderwidth=3, width=6,
